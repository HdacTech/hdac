...
{
    return Read(make_pair('p', key), value);
}
...
bool CBlockTreeDB::UpdateSpentIndex(const std::vector<std::pair<CSpentIndexKey, CSpentIndexValue> > &vect)
{
    ...
    for (std::vector<std::pair<CSpentIndexKey,CSpentIndexValue> >::const_iterator it=vect.begin(); it!=vect.end(); it++) {
        if (it->second.IsNull()) {
            batch.Erase(make_pair('p', it->first));
        } else {
            batch.Write(make_pair('p', it->first), it->second);
        }
    }
    return WriteBatch(batch);
}
...
bool CBlockTreeDB::UpdateAddressUnspentIndex(const std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > &vect)
{
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=vect.begin(); it!=vect.end(); it++) {
        if (it->second.IsNull()) {
            batch.Erase(make_pair('u', it->first));
        } else {
            batch.Write(make_pair('u', it->first), it->second);
        }
    }
    return WriteBatch(batch);
}
...
bool CBlockTreeDB::ReadAddressUnspentIndex(uint160 addressHash, int type, std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > &unspentOutputs)
{
    ...
    ssKeySet << make_pair('u', CAddressIndexIteratorKey(type, addressHash));
    ...
    while (pcursor->Valid()) {
        ...
        try {
            ...
            ssKey >> chType;
            ssKey >> indexKey;
            if (chType == 'u' && indexKey.hashBytes == addressHash) {
                try {
                    leveldb::Slice slValue = pcursor->value();
                    CDataStream ssValue(slValue.data(), slValue.data()+slValue.size(), SER_DISK, CLIENT_VERSION);
...
{
    CLevelDBBatch batch;
    batch.Write(make_pair('S', timestampIndex), 0);
    return WriteBatch(batch);
}
...
bool CBlockTreeDB::ReadTimestampIndex(const unsigned int &high, const unsigned int &low, const bool fActiveOnly, std::vector<std::pair<uint256, unsigned int> > &hashes)
{
    ...
    CDataStream ssKeySet(SER_DISK, CLIENT_VERSION);
    ssKeySet << make_pair('S', CTimestampIndexIteratorKey(low));
    pcursor->Seek(ssKeySet.str());
    ...
    while (pcursor->Valid()) {
        ...
        try {
            ...
            ssKey >> chType;
            ssKey >> indexKey;
            if (chType == 'S' && indexKey.timestamp < high) {
                if (fActiveOnly) {
...
{
    CLevelDBBatch batch;
    batch.Write(make_pair('z', blockhashIndex), logicalts);
    return WriteBatch(batch);
}
...
bool CBlockTreeDB::ReadTimestampBlockIndex(const uint256 &hash, unsigned int &ltimestamp)
{
    ...
    if (!Read(std::make_pair('z', hash), lts))
        return false;
