...
public:
    ...
    void Check()
    {
#ifdef DEBUG_ADDRMAN
        {
            ...
            int err;
            if ((err=Check_()))
                if(fDebug>0)LogPrintf("ADDRMAN CONSISTENCY CHECK FAILED!!! err=%i\n", err);
        }
#endif
    }
    ...
    bool Add(const CAddress &addr, const CNetAddr& source, int64_t nTimePenalty = 0)
    {
        ...
        if (fRet)
            if(fDebug>1)LogPrint("addrman", "Added %s from %s: %i tried, %i new\n", addr.ToStringIPPort(), source.ToString(), nTried, nNew);
        return fRet;
    }
    ...
    bool Add(const std::vector<CAddress> &vAddr, const CNetAddr& source, int64_t nTimePenalty = 0)
    {
        ...
        if (nAdd)
            if(fDebug>1)LogPrint("addrman", "Added %i addresses from %s: %i tried, %i new\n", nAdd, source.ToString(), nTried, nNew);
        return nAdd > 0;
    }
